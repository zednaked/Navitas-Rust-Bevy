...
fn main() {
    ...
    app.add_systems(Update, particle_system.run_if(in_state(AppState::InGame)));
    app.add_systems(Update, pause);
    app.add_systems(Update, move_particles.run_if(in_state(AppState::InGame)));
    app.add_systems(Update, loucura.run_if(in_state(AppState::InGame)));
    app.run();
}
...
)
{
    ...
    //transform.scale.y  += (time * 5.0).sin() /800. ;
    //transform.scale.x  -= (time * 5.0).sin() /800. ;

    let mut projection = query.single_mut();
    projection.scale += ((time * 5.0).sin() / 400.);
...
fn setup (mut cmd: Commands, mut windows: Query  <&mut Window>){ 
    ...
    my_2d_camera_bundle.transform.scale = vec3(1.1,1.1,1.1);
   ...
   // window.prevent_default_event_handling = false;
    ...
    cmd.spawn((Text2dBundle {
        ...
        transform: Transform::from_translation(Vec3::new(0.,380.,0.)),
        ..default() 
    }
    ));
...
fn pause (keys: Res<ButtonInput<KeyCode>>, mut app_state: ResMut<NextState<AppState>>, app_state_atual: Res<State<AppState>>)
{
...
fn move_player ( 
   ...
   if direction == Vec2::ZERO {
        ...
        for mut transform in &mut query {
            ...
            if (transform.translation.x < -150.0) {
            ...
            if (transform.translation.y < -410.0) {
                transform.translation.y = -410.0;
            }
...
impl Plugin for SpawnerEstrelas {
    fn build(&self, app: &mut App) {
        app.add_systems(Update, spawna_estrelas.run_if(in_state(AppState::InGame)))
...
impl Plugin for EstrelasPlugin {
    fn build(&self, app: &mut App) {        
        ...
        app.add_systems(Update, move_estrelas.run_if(in_state(AppState::InGame)));
    }
}
...
impl Plugin for SpawnerAsteroide {
    fn build(&self, app: &mut App) {
        ...
        app.add_systems(Update, spawna_asteroides.run_if(in_state(AppState::InGame)))
...
impl Plugin for AsteroidesPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.add_systems(Update, despawna_asteroides.run_if(in_state(AppState::InGame)));
        app.add_systems(Update, move_asteroides.run_if(in_state(AppState::InGame)));
    }
}
...
impl Plugin for CollisionPlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(Update, check_collision.run_if(in_state(AppState::InGame)));
    }
}
...
impl Plugin for SpawnerBullet {
    fn build(&self, app: &mut App) {
        let mut rgn = thread_rng();
        app.add_systems(Update, spawna_bullets.run_if(in_state(AppState::InGame)) )
...
impl Plugin for BulletPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.add_systems(Update, despawna_bullets.run_if(in_state(AppState::InGame)));
        app.add_systems(Update, move_bullets.run_if(in_state(AppState::InGame)));
    }
}
    ...
    for event in events.read() {
        ...
        commands.spawn((
            ...
            MaterialMesh2dBundle {
                mesh: Mesh2dHandle(meshes.add(Circle { radius: rng.gen_range(35.5..45.0) })),
                material: materials.add(Color::hsl(rng.gen_range(71.5..98.0), 0.9, 0.9)),
...
fn move_particles(
    ...
    for (entity, mut particle, mut transform,mut  mesh) in &mut query {
...
pub struct Inimigo{
    pub speed: f32,
    pub velociodade_atual: f32, //esse valor ganha velocidade a cada ciclo, até chegar na velocidade maxima, se direction for zero ele desacelera
    pub direction: Vec2, //a cada ciclo, ele tem 10% de chance de mudar de direção
    pub velocidade_maxima: f32,
}

#[derive(Resource)]
struct TimerInimigo(Timer); //este é o temporizador responsavel por criar os inimigos

#[derive(Resource)]
struct TimerInimigoMove(Timer); //este é o temporizador responsavel por mover os inimigos

#[derive(Resource)]
struct TimerInimigoShoot(Timer); //este é o temporizador responsavel por fazer os inimigos atirar
